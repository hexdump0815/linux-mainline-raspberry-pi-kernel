...
./scripts/config --set-str CONFIG_LOCALVERSION "-stb-rpi"

for i in `cat /compile/doc/stable-rp/misc.rpi/options/additional-options-*-yes.txt`; do
  echo $i
  ./scripts/config -e $i
done

for i in `cat /compile/doc/stable-rp/misc.rpi/options/additional-options-*-mod.txt`; do
  echo $i
  ./scripts/config -m $i
done
